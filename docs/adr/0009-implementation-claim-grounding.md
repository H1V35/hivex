...
context when an omitted source or unchanged dependency is needed. A caller can then supply additional
source IDs or revise the review scope. No implicit model retry, automatic context truncation or global
consistency claim is allowed.

The model assesses every supplied changed file and source exactly once, including relevance and
...
A definitive verdict needs cited relevant code and documentation, complete coverage, sufficient
...
a new corresponding assessment. A checksum is an integrity binding, not proof of authorship.

Bound each operation to 32 complete changed text files, 16 complete sources, 512 claims, 256 selected
