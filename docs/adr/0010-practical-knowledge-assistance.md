...
The retired TypeScript runtime remains in Git history. Active tests use the Rust CLI, synthetic protocol server and existing v1 SQL fixtures. Internal tests remain only where they protect a meaningful domain contract. Bun and TypeScript are development tools for these tests and packaging, not runtime requirements. Further native targets require their own build, behavior and artifact validation before inclusion.
