...
After validating adoption in two consuming projects, the owner approved replacing the TypeScript/Bun runtime with Rust. This supersedes the language choice above while retaining the product contract and cohesive domain modules. The migration first introduced local document, initialization and recovery commands, then knowledge storage and native execution, and finally npm distribution. The published TypeScript CLI served as the compatibility reference during implementation. The complete Rust CLI was verified and integrated in [PR 98](https://github.com/H1V35/hivex/pull/98); the distribution stage retires the TypeScript runtime and retains its fixed compatibility fixtures.
...
### Native artifact and retirement

The native distribution keeps the `@h1v35/hivex` package, the `hivex` command, all six skills and optional language templates. For the first verified macOS ARM64 target, npm links directly to the executable; no JavaScript launcher, runtime package dependency or installation hook is needed. The package declares its operating system and CPU explicitly. This restricts the native artifact to that validated target; it does not claim compatibility for other platforms previously able to install the Bun package.

The retired TypeScript runtime remains in Git history. Active tests use the Rust CLI, synthetic protocol server and existing v1 SQL fixtures. Internal tests remain only where they protect a meaningful domain contract. The first native delivery retained Bun and TypeScript for tests and packaging; the development-toolchain amendment below supersedes that temporary boundary. Further native targets require their own build, behavior and artifact validation before inclusion.

## Complete Rust development toolchain (#80, owner clarification 2026-09-17)

The owner clarified that complete migration also removes TypeScript, JavaScript and Bun from Hivex development. The earlier runtime-only completion did not satisfy that scope. Tests, synthetic protocol execution, archive preparation and artifact verification now belong to the Cargo/Rust toolchain. Preserve useful CLI, v1 storage, accounting, failure and recovery contracts when consolidating the earlier suite; test counts and language statistics are not acceptance criteria.

Retire the replaced source files, JavaScript dependency lockfile and compiler/linter/formatter configuration rather than hiding them from repository language statistics. Keep fixed SQL compatibility data and Git history. The optional TypeScript Markdown template serves adopting projects and remains part of the package; it is not executable development tooling.

Keep npm as the distribution channel and `package.json` as package metadata. Cargo builds the admitted native target, a Rust utility stages the public allowlist and invokes system `tar`, and verification extracts the exact archive into a temporary project and exercises its executable without Bun/Node on PATH. npm installation/publication remains a separate registry compatibility/release step; changing the development tooling does not authorize republishing an existing version or publishing a new artifact.
