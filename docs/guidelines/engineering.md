...
Use Cargo with the committed lockfile for the native runtime. Use `bun install` for JavaScript/TypeScript development tooling and `bun ci` for its frozen installation. The installed native CLI has no Bun or Node.js dependency and no custom installer or registry verifier. The earlier [Bun installation decision](../adr/0003-independent-bun-installation.md) remains historical context; its runtime requirement is superseded by the Rust distribution.
...
The Rust dependency policy follows the domain modules: document discovery and Markdown parsing do not import knowledge, review or model execution; knowledge storage and ingestion may use documents; review may use documents and knowledge. Model execution is self-contained, retrieval may use shared errors, and CLI commands compose these responsibilities. Rust unit tests may use private production modules; production cannot import tests. JavaScript support scripts and CLI fixture helpers remain separate from the native runtime. ESLint boundaries classify those support files by their actual responsibilities.

Tooling settings identify Bun globals and the simulated server executable while keeping the preset rules enabled. The JavaScript compatibility target is server-side tooling. Test and packaging APIs must be verified on the supported tooling versions.

Rust uses the stable toolchain selected in `rust-toolchain.toml`, a committed Cargo lockfile, rustfmt, Clippy with warnings denied and relevant behavioral checks. Keep Rust modules cohesive and private by default. The TypeScript lint configuration applies to test and support code; do not translate its stylistic thresholds into Rust rules.
