...
Use Cargo with the committed lockfile for the runtime, development tools and tests. The stable toolchain in `rust-toolchain.toml`, rustfmt, Clippy with warnings denied and behavioral checks form the development gate. TypeScript, JavaScript and Bun are retired from Hivex development under the owner's clarification of [#80](https://github.com/H1V35/hivex/issues/80). The earlier [Bun installation decision](../adr/0003-independent-bun-installation.md) is historical context.

Keep Rust modules cohesive and private by default. Document discovery and Markdown parsing do not import knowledge, review or model execution; knowledge storage and ingestion may use documents; review may use documents and knowledge. Model execution is self-contained, retrieval may use shared errors, and CLI commands compose these responsibilities. Unit tests may use private production modules; production cannot import tests. CLI integration tests use the compiled executable, fixed v1 SQL fixtures and a synthetic Rust app-server. The development package utility and synthetic server are separate binaries and are never distributed.

Do not translate the former TypeScript stylistic thresholds into Rust rules. The optional TypeScript documentation template remains applicable to adopting TypeScript projects; it does not configure Hivex development. Repository Markdown remains human-maintained authority. Check changed links, commands and scope without retaining a JavaScript formatter solely for documentation.
