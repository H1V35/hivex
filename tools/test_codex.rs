...
fn response(prompt: &str) -> Value {
    ...
    if packet["operation"] == "extract" && responses["fromVisibleRules"] == true {
        ...
        return json!({"decisions":decisions,"relationships":[],"uncertainties":[]});
    }
    if packet["operation"] == "extract" && responses["byDocument"].is_object() {
                ...
                .unwrap_or_default()
            {
                ...
                for key in ["from", "to"] {
                    if let Some(document) = relationship[key]
                        ...
                        && let Some(existing) = packet["existing"].as_array().and_then(|values| {
                            values.iter().find(|value| value["document"] == document)
                        })
                    {
        ...
        return json!({"decisions":decisions,"relationships":relationships,"uncertainties":[]});
    }
    let mut result = responses[packet["operation"].as_str().unwrap_or_default()].clone();
    if packet["operation"] == "check"
        && let Some(changes) = result["relationshipChanges"].as_array_mut()
    {
    ...
    fn handle(&self, method: &str, params: &Value, events: &mut Vec<Value>) -> Option<Value> {
        ...
        Some(match method {
            ...
            "config/read" => {
                ...
                let mcp = if ["configured-mcp", "ignored-mcp", "abort-isolation"]
                    .contains(&scenario)
                {
            ...
            "turn/start" => {
                    ...
                    && let Ok(contents) = fs::read_to_string(&hold)
                {
                    if contents == "after-first"
                        && fs::read_to_string(environment("HIVEX_TEST_CALLS_PATH"))
                            .unwrap_or_default()
                            == "called\n"
                    {
                        break;
                ...
                json!({"turn":{"id":"turn1","status":"inProgress"}})
            }
    ...
    for line in io::stdin().lock().lines() {
        ...
        if frame["method"] == "initialize" {
            let malformed = match server.scenario.as_str() {
                ...
                "rpc-two-outcomes" => Some(
                    json!({"id":frame["id"],"result":{},"error":{"code":-1,"message":"failure"}}),
                ),
                "rpc-remote-error" => Some(
                    json!({"id":frame["id"],"error":{"code":-32602,"message":"synthetic failure"}}),
                ),
                "rpc-partial-eof" => {
                    print!("{{broken");
                    io::stdout().flush().unwrap();
