...
const options = Object.fromEntries(
  process.argv.flatMap((argument, index, argumentList) => {
    ...
    return [
      [
        setting.slice(firstIndex, split),
        setting.slice(split + nextArgumentOffset),
      ],
    ];
  })
);
const disabledArgument = function disabledArgument(
  argument,
  index,
  argumentList
) {
...
const responseForPrompt = function responseForPrompt(prompt) {
  if (process.env.HIVEX_TEST_RESPONSES) {
    const responses = JSON.parse(
      readFileSync(process.env.HIVEX_TEST_RESPONSES, "utf-8")
    );
    const packet = payloadFromPrompt(prompt);
    if (responses.capturePackets) {
      appendFileSync(
        `${process.env.HIVEX_TEST_RESPONSES}.packets`,
    ...
    if (packet?.operation === "extract" && responses.fromVisibleRules) {
      const visibleDecisions = function visibleDecisions(documentEntry) {
        ...
        const createVisibleDecision = function createVisibleDecision([
          number,
          line,
        ]) {
          return {
        ...
        return documentEntry.lines
          .filter(isVisibleRule)
          .map(createVisibleDecision);
      };
      ...
      const decisions = packet.documents
        .filter(isTargetDocument)
        .flatMap(visibleDecisions);
      return { decisions, relationships: [], uncertainties: [] };
    }
    if (packet?.operation === "extract" && responses.byDocument) {
      const targets =
        packet.targets ?? packet.documents.map((document) => document.id);
          ...
          ...Object.fromEntries(
            ["from", "to"].map((key) => {
              ...
              const isExistingDecision = function isExistingDecision(decision) {
                return (
                  decision.document === endpoint.slice(endpointPrefixLength)
                );
              };
              return [
                key,
...
const handlers = {
  ...
  "config/read": function configRead() {
    const features = Object.fromEntries([
      ...
      [
        ...
        process.argv.includes("skip_host_skill_discovery"),
      ],
    ]);
    const mcpServers = [
      "configured-mcp",
      "ignored-mcp",
      "abort-isolation",
    ].includes(process.env.HIVEX_TEST_SCENARIO)
      ? Object.fromEntries([
          [
            "hivex_extra",
            Object.fromEntries([
  ...
  "thread/start": (parameters) => {
    ...
    const effort =
      process.env.HIVEX_TEST_SCENARIO === "changed-effort" ? "high" : "max";
    return {
      cwd: process.cwd(),
      instructionSources:
  ...
  "turn/start": (parameters) => {
    ...
    if (
      ["start-unconfirmed", "update-uncertain"].includes(
    ...
    if (
      ["timeout", "timeout-unmeasured", "cancel"].includes(
        process.env.HIVEX_TEST_SCENARIO
      )
    ) {
    ...
    const responseCandidate = responseForPrompt(
      parameters.input[firstIndex].text
    );
    queueMicrotask(() => {
      emit({
        method: tokenUsageUpdatedMethod,
...
const waitForHoldRelease = (holdPath, callsPath) => {
  ...
  const check = () => {
    ...
    if (
      holdContents === null ||
      (holdContents === "after-first" &&
        readFileSync(callsPath, "utf-8") === "called\n")
    ) {
      released.resolve();
      return;
    }
