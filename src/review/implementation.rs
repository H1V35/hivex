...
use crate::documents::{line_content, raw_markdown_lines};
...
fn check_size(size: usize, limit: usize) -> Result<()> {
  if size > limit {
    return Err(implementation_too_large(format!(
      "Implementation exceeds {limit} bytes; split the change into coherent reviews."
    )));
  }
  Ok(())
}
...
fn base64(value: &[u8]) -> String {
  ...
  for chunk in value.chunks(3) {
    ...
    output
      .push(TABLE[((first & 0x03) << 4 | second.map_or(0, |value| value >> 4)) as usize] as char);
    if let Some(second) = second {
      let third = chunk.get(2).copied();
      output
        .push(TABLE[((second & 0x0f) << 2 | third.map_or(0, |value| value >> 6)) as usize] as char);
...
fn version(bytes: Vec<u8>, label: &str, warnings: &mut Vec<String>) -> Result<Option<Version>> {
  if bytes.len() > MAX_FILE_BYTES {
    return Err(implementation_too_large(format!(
      "Implementation exceeds {MAX_FILE_BYTES} bytes; split the change into coherent reviews."
    )));
  }
  let content = match String::from_utf8(bytes) {
    Ok(content) => content,
    Err(error) => return Ok(unsupported_version(error.as_bytes(), label, warnings)),
  };
  if content.contains('\0') {
    return Ok(unsupported_version(content.as_bytes(), label, warnings));
  }
  Ok(Some(Version {
    lines: raw_markdown_lines(&content)
      ...
      .collect(),
    version: digest(content.as_bytes()),
  }))
}
...
fn before_version(
  ...
  let Some((header, _)) = row.split_once('\t') else {
    warnings.push(format!(
      "Unsupported base file: {path} (undefined undefined)"
    ));
    return Ok(None);
  };
  ...
  if !mode.starts_with("100") {
    warnings.push(format!("Unsupported base file: {path} ({mode} {object})"));
    return Ok(None);
  }
  let bytes = run_git(
    root,
  ...
  object.insert(
    ...
    Value::Array(
      value
        .lines
        .iter()
        .map(|(number, text)| Value::Array(vec![Value::from(*number), Value::String(text.clone())]))
        .collect(),
    ),
  );
  Value::Object(object)
...
pub fn capture_implementation(root: &Path, base: &str) -> Result<Value> {
  ...
  let fingerprint = digest(
    serde_json::to_string(&fingerprint_value(&base_commit, &diff, &files, &warnings))?.as_bytes(),
  );
...
mod tests {
  ...
  fn root() -> PathBuf {
    let root = std::env::temp_dir().join(format!("hivex-implementation-{}", uuid::Uuid::new_v4()));
  ...
  fn bounds_large_file_context_to_changed_hunks() {
    ...
    for index in 0..2_000 {
      writeln!(before, "const line{index} = 1;").unwrap();
    }
