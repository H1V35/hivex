...
mod system_collation {
  ...
  pub(super) fn compare(left: &str, right: &str) -> std::cmp::Ordering {
    ...
    let right_length = i32::try_from(right.len()).expect("right path exceeds ICU's byte length");
    let mut status = 0;
...
fn collect_candidates(
  ...
  let entries =
    fs::read_dir(current).and_then(std::iter::Iterator::collect::<std::io::Result<Vec<_>>>);
  ...
  for entry in entries {
    ...
    if file_type.is_dir() && !is_excluded_subtree(&path, config) {
...
fn percent_decode(value: &str) -> Option<String> {
  ...
  while index < bytes.len() {
    if bytes[index] == b'%' {
      ...
      let high = u8::try_from(char::from(bytes[index + 1]).to_digit(16)?).ok()?;
      let low = u8::try_from(char::from(bytes[index + 2]).to_digit(16)?).ok()?;
