...
impl Work {
    ...
    pub fn status(&self) -> State {
        State::parse(self.value["status"].as_str().unwrap_or_default())
            .expect("validated work state")
    }
    pub fn phase(&self) -> Phase {
        Phase::parse(self.value["phase"].as_str().unwrap_or("update"))
            .expect("validated work phase")
    }
...
impl Work {
    ...
    pub fn set_remaining(&mut self, units: Vec<String>) {
        self.value["remaining"] = json!(units);
    }
    ...
    pub fn assess_retained_check(&mut self, accepted: bool) {
        self.value["retainedCheckAssessment"] =
            json!(if accepted { "accepted" } else { "blocked" });
    }
...
impl Work {
    pub fn retry_failed(&mut self, requested: bool) -> Result<bool> {
        ...
        if !confirmed
            && last["recoveryAcknowledgement"]["type"] != "uncertain-invocation"
            && !before
        {
            return Err(HivexError::new(
                "WORK_UNCERTAIN",
