...
use crate::knowledge::warning_baseline;
use crate::knowledge::{self as knowledge_model, graph_value, parse_graph};
...
impl UpdateLease {
  fn acquire(directory: &Path) -> Result<Self> {
    ...
    if let Err(error) = file.write_all(token.as_bytes()).and_then(|()| file.flush()) {
...
impl Store {
  ...
  pub fn graph(&self) -> Result<Value> {
    ...
    let Some(data) = data else {
      ...
      return crate::knowledge::shared_knowledge(root);
    };
  ...
  fn begin_bound(
    ...
    let work = resume_work(
      &transaction,
      previous.expect("previous exists after new-work branch"),
      options,
      previous_reusable,
    )?;
    transaction.commit()?;
  ...
  pub fn reserve(&mut self, work: &mut Work, input: super::AttemptInput<'_>) -> Result<()> {
    let transaction = self
      .database
      .transaction_with_behavior(TransactionBehavior::Immediate)?;
    ...
    next.reserve_attempt(input.bytes, input.hash, input.stage)?;
...
fn validate_work_value(value: &Value, row_id: &str) -> Result<()> {
  ...
  validate_work_identity_and_budget(object, row_id)?;
  ...
  validate_retained_progress(object, row_id)?;
  Ok(())
}
...
fn array_of_attempts(object: &Map<String, Value>, row_id: &str) -> Result<()> {
  ...
  for attempt in attempts {
    validate_attempt(attempt, row_id)?;
  }
  Ok(())
}
...
fn validate_extraction(value: &Value, row_id: &str) -> Result<()> {
  ...
  for decision in decisions {
    validate_pending_decision(decision, row_id)?;
  }
  ...
  for relationship in relationships {
    validate_pending_relationship(relationship, row_id)?;
  }
...
fn validate_warning_baseline(object: &Map<String, Value>, row_id: &str) -> Result<()> {
  ...
  if baseline
    .values()
    .any(|value| !matches!(value.as_str(), Some("active" | "resolved")))
  {
    return Err(malformed_work(row_id, "warningBaseline is invalid"));
  }
  Ok(())
...
fn validate_execution_profile(object: &Map<String, Value>, row_id: &str) -> Result<()> {
...
fn validate_attempt(attempt: &Value, row_id: &str) -> Result<()> {
  ...
  if let Some(acknowledgement) = attempt.get("recoveryAcknowledgement") {
    ...
    if acknowledgement
      ...
      .and_then(Value::as_str)
      .is_none()
      || acknowledgement.get("type").and_then(Value::as_str) != Some("uncertain-invocation")
      || acknowledgement
        .get("nativeProcessId")
        .and_then(as_positive_integer)
        .is_none()
...
fn validate_pending_decision(decision: &Value, row_id: &str) -> Result<()> {
  ...
  explanation_array(decision, "conditions", row_id, "pending decision")?;
  explanation_array(decision, "exceptions", row_id, "pending decision")?;
  Ok(())
}

fn validate_pending_relationship(relationship: &Value, row_id: &str) -> Result<()> {
...
fn validate_work_identity_and_budget(object: &Map<String, Value>, row_id: &str) -> Result<()> {
...
fn previous_work(transaction: &Connection, options: &BeginWork) -> Result<Option<Work>> {
  let previous = transaction
    .prepare("SELECT id,data FROM work WHERE kind=?1 AND key=?2 ORDER BY rowid DESC LIMIT 1")?
...
mod tests {
  ...
  fn validates_graphs_and_reads_shared_snapshot_when_local_graph_is_absent() {
    ...
    crate::knowledge::write_knowledge_snapshot(&root, &graph).expect("write shared snapshot");
    let mut store = Store::open(&root, StoreOptions::default()).expect("open store");
  ...
  fn resumes_work_without_resetting_counters_or_optional_field_shape() {
    ...
    let mut work = store.begin(&options).expect("begin");
    assert_eq!(work.calls(), 0);
    store
      .reserve(
        &mut work,
        crate::work::AttemptInput {
          bytes: 40,
          hash: "input-1",
          stage: "extract",
        },
    ...
    store
      ...
      .expect("record pid");
    work
      .record_completion(crate::work::Completion {
        report: json!({"outcome":"failed","cleanup":"confirmed","turnAccepted":"confirmed","usage":null}),
        result: None, output_hash: None, invalid_output: None,
      })
      .unwrap();
    work
      .value_mut()
  ...
  fn reuses_completed_update_only_for_the_same_empty_plan_and_resets_ask_result() {
    ...
    let mut completed = store.begin(&update).expect("begin update");
    completed
      .value_mut()
      .as_object_mut()
      .expect("object")
    ...
    let mut answer = store.begin(&ask).expect("begin ask");
    answer
      .value_mut()
      .as_object_mut()
      .expect("object")
