...
use crate::work::store::{self as store, LOCK_FILENAME, Store};
...
fn recover_checked(store: &mut Store, acknowledge_uncertain: bool) -> Result<RecoveryReport> {
  ...
  for (index, work) in works.iter().enumerate() {
    ...
    if work.status == "failed"
      && let Some(report) = uncertain_failed_report(work)
    {
      uncertain_failed.push(RecoveryCandidate {
        index,
        native_pid: report
          .get("nativeProcessId")
...
fn blocked_recovery(error: HivexError) -> RecoveryReport {
  let lock = error
    ...
    .map_or("unreadable", |lock| match lock {
      "absent" => "absent",
      "held" => "held",
      "changed" => "changed",
      _ => "unreadable",
...
fn release_recovery_lock(
  ...
  let released = match fs::read_to_string(&path) {
    Ok(contents) if contents == lock.raw => fs::remove_file(&path).map(|()| true),
    Ok(_) => Ok(false),
...
fn record_recovery(
  ...
  let result = (|| -> Result<()> {
    for candidate in candidates {
      ...
      if current.status().as_str() != work.status
        || i64::try_from(current.calls()).ok() != Some(work.calls)
      {
        return Err(recovery_unsafe(
          "changed",
          &format!(
...
fn uncertain_failed_report(work: &WorkSnapshot) -> Option<&Map<String, Value>> {
...
fn prune(store: &mut Store, keep_completed: usize, keep_caches: usize) -> Result<PruneReport> {
  ...
  let cache_rows: Vec<i64> = {
    ...
    let mut statement = database.prepare("SELECT rowid FROM model_cache ORDER BY rowid DESC")?;
    statement
...
pub fn maintain(storage: &mut Store, operation: Maintenance) -> Result<Value> {
  match operation {
    Maintenance::Recover {
      acknowledge_uncertain,
    } => Ok(recovery_value(
      "recover",
      recover(storage, acknowledge_uncertain)?,
    )),
    Maintenance::Prune {
      keep_completed,
      keep_caches,
    } => {
      let report = prune(storage, keep_completed, keep_caches)?;
      Ok(json!({
        "command": "prune", "modelCalls": 0,
