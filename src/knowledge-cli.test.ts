...
const isRelationship = function isRelationship(
  value: unknown
): value is ModelRelationship {
  return (
    isRecord(value) &&
...
const isModelResponses = function isModelResponses(
  value: unknown
): value is ModelResponses {
  ...
  return (
    Array.isArray(value.extract.relationships) &&
    value.extract.relationships.every(isRelationship)
  );
};
...
const isDocumentResponseMap = (
  value: unknown
): value is Record<string, ModelDocumentResponse> =>
  isRecord(value) && Object.values(value).every(isDocumentResponse);

const isModelReview = function isModelReview(
  value: unknown
): value is ModelReview {
  return (
    isRecord(value) &&
    isStringArray(value.uncertainties) &&
...
): Record<string, ModelDocumentResponse> => {
...
const isModelPacket = function isModelPacket(
  value: unknown
): value is ModelPacket {
  if (!isRecord(value)) {
    return false;
  }
  return (
...
const isSnapshotGraph = function isSnapshotGraph(
  value: unknown
): value is SnapshotGraph {
  return (
    isRecord(value) &&
    isRecordArray(value.decisions) &&
    isRecordArray(value.relationships)
  );
};
...
const workId = (value: unknown): string =>
  stringField(objectField(value, "work"), "id");

const workCalls = (value: unknown): number =>
  numberField(objectField(value, "work"), "calls");
...
const legacyWorkFixtures: LegacyWorkFixture[] = [
  {
    ...
    snapshot:
      "a82b745c845cc5f7cc34d71a796ffb2894f3478a3e522836456fe47eff88eea5",
...
await import(${JSON.stringify(
      pathToFileURL(
        nodePath.join(import.meta.dirname, "../test/codex-server.mjs")
      ).href
    )});
`
  );
...
test("keeps declared history out of an ordinary update plan", () => {
  ...
  try {
    ...
    expect(arrayField(response.value, "pendingDocuments")).not.toContain(
      "archive/replaced.md"
    );
  } finally {
    rmSync(root, { force: true, recursive: true });
...
test("resumes a checked knowledge batch without repeating its extraction or resetting its budget", () => {
  project((root) => {
    ...
    expect(
      readFileSync(nodePath.join(root, "model-calls.log"), "utf-8")
        .trim()
        .split("\n")
    ).toHaveLength(1);
...
test("discovers a cross-batch exception and follows its indirect dependents within the query limit", () => {
  project((root) => {
    ...
    const bounded = invoke(root, [
      "neighbors",
      decisionId(found.value),
      "--limit",
      "2",
    ]);
...
test("keeps a decision with a bad line reference uncertain but can consult its original document", () => {
  project((root) => {
    ...
    expect(invoke(root, ["update", "--codex", binary]).value.status).toBe(
      "partial"
    );
...
test("asks a declared historical source with bounded evidence and historical provenance", () => {
  project((root) => {
    ...
    expect(stringField(answer.value, "answer")).toBe(
      "The old cache rule allowed seven days."
    );
    ...
    const packets = readFileSync(`${file}.packets`, "utf-8")
      .trim()
      .split("\n")
      .map(parsePacket);
    const ordinary = packets.findLast(
      (packet) => packet.operation === "extract"
    );
    if (!ordinary) {
...
test("does not ingest a historical match during an ordinary consultation", () => {
  project((root) => {
    ...
    expect(arrayField(answer.value, "pendingDocuments")).not.toContain(
      "archive/replaced.md"
    );
    writeFileSync(
      nodePath.join(root, "archive/replaced.md"),
...
test("names an unconsulted historical reference instead of silently approving incomplete context", () => {
  project((root) => {
    ...
    expect(answer.value.status).toBe("partial");
    expect(JSON.stringify(arrayField(answer.value, "warnings"))).toContain(
...
test("retrieves a consulted historical dependency when a current query reaches it", () => {
  project((root) => {
        ...
        relationships: [
          {
            ...
            id: "history-to-cache",
            reason:
              "The current rule replaced the historical retention period.",
    ...
    responses.ask = {
      answer:
        "The current cache rule superseded the historical seven-day rule.",
      evidence: [{ document: "archive/replaced.md", lineEnd: 3, lineStart: 3 }],
      uncertainties: [],
    };
    writeFileSync(file, JSON.stringify(responses));
    ...
    project((clone) => {
      ...
      for (const sourceFile of [
        ...
        copyFileSync(
          nodePath.join(root, sourceFile),
          nodePath.join(clone, sourceFile)
        );
      }
      ...
      expect(arrayField(reused.value, "pendingDocuments")).not.toContain(
        "archive/unread.md"
      );
      expect(existsSync(nodePath.join(clone, ".hivex/knowledge.sqlite"))).toBe(
        false
      );
    ...
    const packetLines = readFileSync(`${file}.packets`, "utf-8")
      .trim()
      .split("\n");
    ...
    expect(JSON.stringify(lastPacket.documents)).toContain(
      "Updated historical explanation."
    );
  });
});
...
test("ingests a large document in resumable rounds without losing earlier decisions", () => {
  project((root) => {
    ...
    const paragraphs = Array.from(
      { length: 140 },
      (...callbackArguments: [undefined, number]) => {
    ...
    responses.ask.evidence = [
      { document: "cache.md", lineEnd: 559, lineStart: 559 },
    ];
    ...
    expect(stringField(answer.value, "answer")).toBe(
      "Rule 139 requires cache expiry."
    );
    expect(arrayField(answer.value, "evidence")).toContainEqual(
...
test("reuses retained model results when previously ingested Markdown is restored", () => {
  project((root) => {
    ...
    expect(workCalls(invoke(root, ["update", "--codex", binary]).value)).toBe(
      2
    );
    writeFileSync(nodePath.join(root, "cache.md"), original);
    const restored = invoke(root, [
      "update",
      "--max-calls",
      "0",
      "--codex",
...
test("invalidates a relationship when its independent supporting document changes", () => {
  project((root) => {
    ...
    writeFileSync(file, JSON.stringify(responses));
    expect(
      invoke(root, ["update", "--codex", binary]).value.relationships
    ).toBe(1);
    const found = invoke(root, ["search", "seven days"]);
    writeFileSync(
      nodePath.join(root, "scope.md"),
...
for (const initialBudget of [1, 2, 16]) {
  test(`restores earlier rounds after an intervening document version (initial budget ${initialBudget})`, () => {
    project((root) => {
      ...
      expect(first.value.status).toBe(
        initialBudget < 16 ? "budget-exhausted" : "ready"
      );
      writeFileSync(
        nodePath.join(root, "cache.md"),
        original.replace("Rule 0", "Rule zero")
      );
      expect(
        invoke(root, [
          "update",
...
test("plans rounds for Markdown larger than the previous two MiB source limit", () => {
  project((root) => {
    ...
    expect(arrayField(response.value, "pendingDocuments")).toContain(
      "large.md"
    );
    expect(arrayField(response.value, "pendingUnits").length).toBeGreaterThan(
      100
    );
...
test("runs maintenance through the CLI without calling the model or discarding graph knowledge", () => {
  project((root) => {
    ...
    expect(arrayField(after.value, "decisions")).toEqual(
      arrayField(before.value, "decisions")
    );
    expect(
      readFileSync(nodePath.join(root, "model-calls.log"), "utf-8")
        .trim()
...
test("checks a retained decision when another snapshot re-extracts the same unit", () => {
  project((root) => {
    ...
    writeFileSync(file, JSON.stringify(responses));
    expect(
      invoke(root, ["update", "--max-calls", "1", "--codex", binary]).value
        .status
    ).toBe("budget-exhausted");
    writeFileSync(
      nodePath.join(root, "note.md"),
...
test("refreshes a changed decision and its known incoming dependency beyond lexical or recent matches", () => {
  project((root) => {
    ...
    writeFileSync(file, JSON.stringify(responses));
    expect(
      invoke(root, ["update", "--max-calls", "4", "--codex", binary]).value
        .status
    ).toBe("ready");
    ...
    responses.ask.answer =
      "Freshness renews every second sunrise; revocation takes priority.";
    writeFileSync(file, JSON.stringify(responses));
...
test("repairs a wrong interpretation without changing Markdown or repeating an identical repair", () => {
  project((root) => {
    ...
    expect(readFileSync(nodePath.join(root, "cache.md"), "utf-8")).toBe(
      original
    );
    expect(workId(invoke(root, commandArguments).value)).toBe(
      workId(repaired.value)
    );
    expect(
      readFileSync(nodePath.join(root, "model-calls.log"), "utf-8")
        .trim()
...
test("keeps a source-local check finding out of an unrelated consultation", () => {
  project((root) => {
    ...
    responses.check.findings = [
      { reason: "The decorative exception is unclear.", target: "c3" },
    ];
    writeFileSync(file, JSON.stringify(responses));
    expect(invoke(root, ["update", "--codex", binary]).value.status).toBe(
      "partial"
...
test("automatic maintenance prioritizes the matching fragment in a large document", () => {
  project((root) => {
    ...
    writeFileSync(file, JSON.stringify(responses));
    expect(
      invoke(root, ["ask", "Quasar", "--max-calls", "2", "--codex", binary])
        .value.status
    ).toBe("budget-exhausted");
...
test("evidence contains source coordinates and text without duplicating decision metadata", () => {
  project((root) => {
    ...
    const evidence = objectField(
      recordAt(
        arrayField(invoke(root, ["search", "seven days"]).value, "decisions"),
        0
      ),
      "evidence"
    );
    expect(
      Object.keys(evidence).toSorted((left, right) => left.localeCompare(right))
    ).toEqual([
      "document",
      "historical",
      "lineEnd",
...
test("repair removes an unsupported relationship even when its endpoints are in unchanged documents", () => {
  project((root) => {
    ...
    responses.check.findings = [
      { reason: "The claimed exception is unsupported.", target: "r1" },
    ];
    writeFileSync(file, JSON.stringify(responses));
    expect(
      invoke(root, ["update", "--codex", binary]).value.relationships
...
test("an endpoint update retains independent evidence and pauses before calls if that context cannot fit", () => {
  project((root) => {
    const supporting =
      "The blue pulse condition governs this exception. ".repeat(3);
    writeFileSync(
      nodePath.join(root, "scope.md"),
    ...
    expect(
      arrayField(
        objectField(objectField(first.value, "work"), "contextLimit"),
        "documents"
      )
    ).toContain("scope.md");
    const resumed = invoke(root, [
    ...
    expect(invoke(root, ["search", "cache"]).value.relationships).toHaveLength(
      1
    );
  });
});
...
test("a document-level omission stays local even when no decision was extracted for that document", () => {
  project((root) => {
    ...
    expect(
      JSON.stringify(invoke(root, ["search", "Amber"]).value.warnings)
    ).toContain("glyph decision");
  });
});
...
test("a changed known supporting document takes priority over unrelated pending documents", () => {
  project((root) => {
    ...
    expect(arrayField(answer.value, "pendingDocuments").length).toBeGreaterThan(
      0
    );
...
const reviewProject = function reviewProject(
  root: string,
  ...
  writeFileSync(
    nodePath.join(root, "cache.ts"),
    "export const purgeOnRevocation = true;\n"
  );
  ...
  writeFileSync(
    nodePath.join(root, "cache.ts"),
    "export const purgeOnRevocation = false;\n"
  );
  const binary = model(root);
  const file = nodePath.join(root, "responses.json");
  const responses = parseModelResponses(readFileSync(file, "utf-8"));
    ...
    findings: [
      {
        ...
        explanation:
          "Private cache survives access revocation, contradicting the purge rule.",
      },
    ],
    uncertainties: [],
  };
...
test("review shares maintenance budget, cites current code and documents, and detects later changes", () => {
  project((root) => {
    ...
    expect(recordAt(arrayField(final.value, "findings"), 0)).toMatchObject({
      assessment: "conflict",
      code: [
        { path: "cache.ts", text: "export const purgeOnRevocation = false;" },
      ],
    });
    expect(
      recordAt(
        arrayField(
          recordAt(arrayField(final.value, "findings"), 0),
          "documents"
        ),
        0
      )
    ...
    expect(
      invoke(root, ["review", "--check", "report.json"]).value.status
    ).toBe("current");
    ...
    expect(
      invoke(root, ["review", "--check", "report.json"]).value
    ).toMatchObject({
    ...
    expect(
      invoke(root, ["review", "--check", "report.json"]).value.documentsChanged
    ).toBe(true);
  });
});
...
test("review keeps an unverifiable finding local while retaining a supported exception", () => {
  project((root) => {
    ...
    const code = [
      { lineEnd: 1, lineStart: 1, path: "cache.ts", side: "after" },
    ];
    const binary = reviewProject(root, {
      findings: [
        {
    ...
    expect(result.value.uncertainties).toContain(
      "The deployment size is not documented."
    );
  });
});
...
test("review binds deleted and untracked code and preserves an omitted resumed call limit", () => {
  project((root) => {
    ...
    expect(
      arrayField(recordAt(arrayField(final.value, "findings"), 0), "code")
    ).toEqual([
      expect.objectContaining({
        path: "cache.ts",
        side: "before",
...
test("review rejects oversized implementation before spending and exposes unsupported binary scope", () => {
  project((root) => {
    ...
    expect(parseError(large.stderr).error.code).toBe(
...
test("shares checked knowledge with a fresh clone without re-extraction or query artifacts", () => {
  project((root) => {
    ...
    expect(
      Object.keys(parseRecord(snapshot)).toSorted((left, right) =>
    ...
    project((clone) => {
      ...
      expect(
        invoke(clone, ["neighbors", decisionId(found.value)]).value.decisions
      ).toEqual(arrayField(neighbors.value, "decisions"));
      const updated = invoke(clone, [
        "update",
        "--max-calls",
        "0",
        "--codex",
      ...
      expect(
        readFileSync(nodePath.join(clone, ".hivex/graph.json"), "utf-8")
      ).toBe(snapshot);
    });
  });
});
...
test("snapshot import preserves unfinished work and only restores graph knowledge after completion", () => {
  project((root) => {
    expect(invoke(root, ["update", "--codex", model(root)]).value.status).toBe(
      "ready"
    );
    expect(invoke(root, ["snapshot", "export"]).status).toBe(0);
    project((clone) => {
      const binary = model(clone);
      const pending = invoke(clone, [
        "update",
        "--max-calls",
        "1",
        "--codex",
      ...
      expect(
        workCalls(invoke(clone, ["update", "--codex", binary]).value)
      ).toBe(2);
      expect(
        readFileSync(nodePath.join(clone, "model-calls.log"), "utf-8")
          .trim()
          .split("\n")
      ).toHaveLength(2);
    });
...
test("shared snapshots expose changed sources and retain reusable neighboring knowledge", () => {
  project((root) => {
    ...
    project((clone) => {
      ...
      expect(
        invoke(clone, ["search", "revocation"]).value.decisions
      ).toContainEqual(
        expect.objectContaining({
          text: "Access revocation immediately purges private cache.",
        })
...
test("rejects invalid snapshot relationships without corrupting local knowledge", () => {
  project((root) => {
    ...
    expect(invoke(root, ["search", "seven days"]).value.decisions).toHaveLength(
      1
    );
    expect(invoke(root, ["snapshot", "export"]).status).toBe(0);
...
test("rejects broken evidence before replacing locally checked knowledge", () => {
  project((root) => {
    ...
    recordAt(arrayField(at(reversed.relationships, 0), "evidence"), 0).lineEnd =
      1;
