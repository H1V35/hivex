...
use crate::knowledge::parse_warning_resolutions;
  ...
  pub fn parse(self, value: &Value) -> Option<Value> {
    match self {
      ...
      Self::Check {
        relationships,
        warnings,
      } => parse_check_output(value, relationships, warnings),
      Self::Answer => Some(
      ...
      Self::Review => {
        ...
        let findings = values.iter().map(|finding| {
                    ...
                    let code = values.iter().map(review_code_citation).collect::<Option<Vec<_>>>()?;
...
fn parse_check_output(value: &Value, relationships: bool, warnings: bool) -> Option<Value> {
  ...
  if warnings {
    result["warningResolutions"] = match value.get("warningResolutions") {
      Some(resolutions) => serde_json::to_value(parse_warning_resolutions(resolutions)?).ok()?,
      None => json!([]),
    };
  }
  Some(result)
...
fn review_code_citation(citation: &Value) -> Option<Value> {
  ...
  if start == 0
    ...
    || !["before", "after"].contains(&side)
  {
    return None;
  }
  Some(
    json!({"lineEnd":end,"lineStart":start,"path":text(normalized.get("path")?,1,usize::MAX)?,"side":side}),
  )
}
