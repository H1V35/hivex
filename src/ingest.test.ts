...
const server = pathToFileURL(join(import.meta.dirname, '../test/codex-server.mjs')).href;
...
async function fixture(
  ...
  const directory = mkdtempSync(join(tmpdir(), 'hivex-ingest-'));
  ...
  try {
    ...
    writeFileSync(calls, '');
    await run({ root, store, binary, calls, hold, scenario, git });
