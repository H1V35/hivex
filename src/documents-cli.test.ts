...
const documentPaths = function documentPaths(value: unknown): string[] {
  return arrayField(value, "documents").map((document) =>
    stringField(document, "path")
  );
};

const documentCommand = function documentCommand(
  cliArguments: string[]
): unknown {
...
test("sources accepts BOM configuration and lists current nested Markdown without returning text", () => {
  ...
  expect(recordAt(arrayField(result, "documents"), 0)).not.toHaveProperty(
    "text"
  );
});
...
test("selects nested documents, preserves exact text, and resolves declared local links", () => {
  ...
  writeFileSync(
    nodePath.join(root, "packages", "module.md"),
    "# Module\n",
    "utf-8"
  );
  writeFileSync(
    nodePath.join(root, "packages", "vendor", "ignored.md"),
    "# Vendor\n",
    "utf-8"
  );
  writeFileSync(
    nodePath.join(root, ".private", "ignored.md"),
    "# Private\n",
    "utf-8"
  );
  ...
  expect(stringField(changed, "snapshot")).not.toBe(
    stringField(listed, "snapshot")
  );
});
...
test("accepts a custom explicit layout and rejects legacy collections without migration", () => {
  ...
  try {
    ...
    writeFileSync(
      nodePath.join(root, "knowledge", "guide.md"),
      "# Guide\n",
      "utf-8"
    );
    writeFileSync(
      nodePath.join(root, "docs", "not-selected.md"),
      "# Other\n",
      "utf-8"
    );
    writeFileSync(
    ...
    expect(() => documentCommand(["sources", "--root", root])).toThrow(
      /legacy collections/u
    );
  } finally {
    rmSync(root, { force: true, recursive: true });
  }
...
test("declares historical globs for focused reads without treating them as current sources", () => {
  ...
  try {
    ...
    expect(stringField(opened, "text")).toBe(
      "# Replaced\n\nHistorical rule.\n"
    );
  } finally {
    rmSync(root, { force: true, recursive: true });
...
test("a large historical archive does not displace current sources at the document limit", () => {
  ...
  for (let index = 0; index < 2048; index += 1) {
    writeFileSync(
      nodePath.join(root, "archive", `${index}.md`),
      "# Old rule\n"
    );
  }
  writeFileSync(
    nodePath.join(root, "current.md"),
    "# Current\n\nCurrent rule.\n"
  );
  writeFileSync(
    nodePath.join(root, "hivex.json"),
...
test("does not make an excluded historical source readable", () => {
  ...
  try {
    ...
    expect(
      arrayField(documentCommand(["sources", "--root", root]), "documents")
    ).toEqual([]);
    expect(() =>
      documentCommand(["read", "docs/archive/replaced.md", "--root", root])
    ).toThrow(/was not selected/u);
  } finally {
    rmSync(root, { force: true, recursive: true });
  }
});
...
test("prunes excluded directories before inspecting their files and symlinks", () => {
  const root = mkdtempSync(
    nodePath.join(tmpdir(), "hivex-excluded-directory-")
  );
  try {
    mkdirSync(nodePath.join(root, "app", "ios"), { recursive: true });
    writeFileSync(
      nodePath.join(root, "app", "docs.md"),
      "# Legitimate\n",
      "utf-8"
    );
    writeFileSync(
      nodePath.join(root, "app", "ios", "README.md"),
      "# Generated\n",
      "utf-8"
    );
    symlinkSync(
      nodePath.join(root, "missing.md"),
      nodePath.join(root, "app", "ios", "broken.md")
    );
    writeFileSync(
      nodePath.join(root, "hivex.json"),
...
test.each([
  ...
  try {
    ...
    writeFileSync(
      nodePath.join(root, "app", "ios", "top.md"),
      "# Top\n",
      "utf-8"
    );
    writeFileSync(
      nodePath.join(root, "app", "ios", "nested", "guide.md"),
      "# Nested\n",
      "utf-8"
    );
    symlinkSync(
...
test("reports invalid UTF-8 and skips symlinked sources without reading outside the root", () => {
  ...
  writeFileSync(
    nodePath.join(root, "docs", "invalid.md"),
    Buffer.from([195, 40])
  );
...
test("pages source discovery and rejects a continuation after the selected documents change", () => {
  ...
  expect(
    arrayField(first, "documents").map((document) =>
  ...
  expect(
    arrayField(second, "documents").map((document) =>
      stringField(document, "id")
    )
  ).toEqual(["b.md"]);
