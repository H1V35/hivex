...
fn source() -> Document {
  ...
  "c6faa807ee055873a73b0e8b823511be81bfaa4b69bc3e141cafc636cfc31c2a".clone_into(&mut source.hash);
  "Private cache".clone_into(&mut source.title);
  source
}
    ...
    &ReviewContext {
      ...
      supplied: &[crate::knowledge::model::SuppliedDocument {
        ...
        lines: vec![
          ...
          vec![
            Value::from(3),
            Value::String("Remove cached private data when access is revoked.".to_owned()),
          ],
        ],
      }],
...
fn decision_identity_uses_utf8_json_with_utf16_length_validation() {
  ...
  "v-é😀".clone_into(&mut source.hash);
  let extraction = Extraction {
...
fn rejects_unsafe_ranges_and_matches_javascript_citation_whitespace() {
  let maximum = usize::try_from(crate::compatibility::MAX_SAFE_INTEGER).unwrap();
  ...
  assert!(
    crate::execution::OutputSchema::Answer
      .parse(&answer)
      .is_none()
  );
...
fn check_targets_preserve_the_scope_and_provenance_of_retained_knowledge() {
  ...
  "retained-neighbor".clone_into(&mut neighbor.id);
  "neighbor.md".clone_into(&mut neighbor.document);
  "previous".clone_into(&mut neighbor.batch);
  "neighbor-local".clone_into(&mut neighbor.local_id);
  ...
  "previous-edge".clone_into(&mut previous_edge.id);
  "previous".clone_into(&mut previous_edge.batch);
  ...
  for target in [&graph.decisions[0].id, &graph.decisions[0].local_id] {
    ...
    assert_eq!(applied.decisions[0].quality, "uncertain");
    assert_eq!(applied.relationships[0].quality, "uncertain");
    assert_eq!(applied.decisions[0].batch, "current");
    assert_eq!(applied.decisions[2].quality, "checked");
