...
use crate::documents::raw_markdown_lines;
...
pub fn parse_warning_resolutions(value: &Value) -> Option<Vec<WarningResolutionInput>> {
  ...
  values
    ...
    .map(|value| {
      let mut resolution =
        serde_json::from_value::<WarningResolutionInput>(normalize_integral_numbers(value.clone()))
          .ok()?;
      ...
      Value::Array(
        resolution
          ...
          .map(|scope| {
            ...
            scope_value.insert("document".to_owned(), Value::String(scope.document.clone()));
...
pub fn warning_review_candidates(
  ...
  let mut candidates: Vec<_> = graph
    ...
    .iter()
    .filter_map(|warning| {
      if matches!(warning, Warning::Legacy(_)) || is_warning_resolved(warning, context.documents) {
        return None;
      }
