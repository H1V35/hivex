    ...
    lines.retain(|line| {
      line[0].as_u64().is_some_and(|line| {
        ranges.iter().any(|range| {
          range.document == id && line >= range.line_start as u64 && line <= range.line_end as u64
        })
      })
    });
  }
...
fn batch_context(
  ...
  let previous = previous_relationships(state, &ranges);
  ...
  let mut missing = context_references(project, &targets, references, &mut ranges);
  ranges.extend(
    project
      .documents
      .iter()
  ...
  let priorities = unique(
    required
      ...
      .chain(
        candidates
          .iter()
          .filter(|entry| linked.contains(&entry.document) && !historical.contains(&entry.document))
          .map(|entry| entry.id.clone()),
      )
  ...
  for id in priorities {
    ...
    if let Some(evidence) = evidence
      && (required.contains(&id) || (existing.len() < 18 && bytes + evidence.text.len() <= 8192))
    {
  ...
  let documents = context_document_packet(project, &ranges);
  Ok(BatchContext {
    documents,
    existing,
    replacing: replacing
...
struct UpdatePlan {
  ...
  original: Vec<IngestionUnit>,
  documents: Vec<Document>,
  range_sources: HashSet<String>,
  ...
  let mut work = match shared {
    ...
    None => store.begin_with_profile(
      &BeginWork {
        ...
        remaining: prepared.remaining.clone(),
        result_key: None,
...
fn plan_update(
  ...
  let SourceGraph { project, graph } = state;
  validate_repair_ranges(state, runtime)?;
  let scoped = shared
    .as_ref()
...
fn validate_repair_ranges(state: SourceGraph<'_>, runtime: &Options) -> Result<()> {
  let SourceGraph { project, graph } = state;
...
impl UpdatePlan {
  fn restore(&mut self, work: &mut Work) -> Result<()> {
    let Self {
      plan,
      original,
      documents,
      ...
      let retained = strings(&work.value()["plannedUnits"])
        ...
        .map(|id| {
          let range = parse_range(id)
            .ok_or_else(|| HivexError::new("INVALID_ARGUMENT", "Invalid retained repair range"))?;
  ...
  let context_ranges: Vec<_> = supplied
    ...
    .flat_map(|document| {
      document
        ...
        .map(|line| Citation {
          document: document.id.clone(),
          line_start: usize::try_from(line).expect("validated source line"),
          line_end: usize::try_from(line).expect("validated source line"),
          version: None,
        })
    })
    .collect();
...
fn add_retained_relationships(
  ...
  if retained_context(pending) {
    ...
    let existing = strings(&pending["existing"]);
    let visible: HashSet<_> = existing
      .iter()
      ...
      .chain(
        candidate
          .decisions
          .iter()
          .filter(|entry| entry.batch == pending["batch"].as_str().unwrap_or_default())
          .map(|entry| entry.id.as_str()),
      )
      .collect();
    packet["retainedRelationships"] = json!(
      candidate
        .relationships
        .iter()
        .filter(|edge| edge.batch != batch
...
fn finish_round(evidence: BatchEvidence<'_>, graph: &mut Graph, work: &mut Work, ids: &[String]) {
  let BatchEvidence { project, plan } = evidence;
  work.set_remaining(
    work
      .remaining()
...
fn extract_batch(
  ...
  {
    if (runtime.repair_ranges.is_empty() && units.len() == 4) || bytes + unit.text.len() > 16_384 {
      break;
    }
    bytes += unit.text.len();
    ...
    .filter(|change| {
      ...
      !impact.is_uncertain_batch
        && check
          .findings
          .iter()
          .all(|finding| finding.target != "batch" && change["previousId"] != finding.target)
...
fn can_review_warnings(candidate: &Graph, pending: &Value, check: &model::KnowledgeCheck) -> bool {
  let documents = strings(&pending["documents"]);
  let invalid = candidate.warnings.iter().any(|warning| match warning {
    model::Warning::Structured(warning) => {
      warning.kind.as_deref() == Some("validation")
        && warning
          .scope
...
fn check_batch(
  ...
  let candidate = if pending["staged"] == true {
    materialize(evidence, graph, &pending, true)?
  } else {
    graph.clone()
  };
  let request = check_request(graph, &candidate, &pending);
  if !admit_check_context(
    &request,
    &pending,
    BatchExecution {
      runtime,
  ...
  if can_review_warnings(&candidate, &pending, &check) {
    checked = complete_warning_review(
      SourceGraph {
        project,
        graph: &checked,
      },
...
fn update_graph(project: &Project, store: &Store) -> Result<Graph> {
...
pub fn update_with_store(
  ...
  while !work.remaining().is_empty() || !work.value()["pending"].is_null() {
    if !pending_current(project, &work.value()["pending"]) {
      let Some(extracted) = extract_batch(
        evidence,
        &graph,
        BatchExecution {
          runtime,
  ...
  if work.remaining().is_empty() && work.value()["pending"].is_null() {
    finish_round(evidence, &mut graph, &mut work, &[]);
    store.commit(&mut work, &model::graph_value(&graph, false))?;
  }
  Ok((update_response(evidence, &graph, &work, runtime), work))
}
...
fn previous_relationships(state: SourceGraph<'_>, ranges: &[Citation]) -> Vec<model::Relationship> {
  let SourceGraph { project, graph } = state;
...
fn context_references(
  ...
  references: impl Iterator<Item = Citation>,
  ranges: &mut Vec<Citation>,
) -> Vec<String> {
  ...
  for citation in references {
    ...
    {
      if targets.contains(&citation.document) && citation.version.as_deref() != Some(&document.hash)
      {
        continue;
      }
...
fn offer_warning_candidates(
  ...
  let available = max_context_bytes.saturating_sub(stringify_knowledge(&sized).len());
...
fn mark_historical_extraction(project: &Project, extraction: &mut Value) {
...
fn historical_and_linked_sources(
  project: &Project,
  targets: &HashSet<String>,
...
fn context_document_packet(project: &Project, ranges: &[Citation]) -> Vec<Value> {
  document_packet(
    project,
...
fn update_identity(runtime: &Options, snapshot: &str, context_sources: Vec<String>) -> Value {
...
fn complete_warning_review(
  ...
  .collect();
  let resolutions =
    warnings::parse_warning_resolutions(value.get("warningResolutions").unwrap_or(&json!([])))
      .ok_or_else(|| HivexError::new("READ_FAILED", "Invalid warning resolutions"))?;
  Ok(warnings::apply_warning_review(
    graph,
...
fn pending_repair_unit(graph: &Graph, runtime: &Options, unit: &IngestionUnit, key: &str) -> bool {
  ...
  let selected = ranges.is_empty()
    || ranges
      .iter()
      .any(|range| range.line_start <= unit.line_end && range.line_end >= unit.line_start);
