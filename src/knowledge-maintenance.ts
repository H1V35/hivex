...
const retention = function retention(
...
export const knowledgeMaintenance = function knowledgeMaintenance(
  ...
  if (
    command === "recover" &&
    (parsed.values["keep-completed"] !== undefined ||
      parsed.values["keep-caches"] !== undefined)
  ) {
    throw new HivexError({
      code: "INVALID_ARGUMENT",
  ...
  return {
    ...
    modelCalls: 0,
    ...store.prune({
      keepCaches: retention(
        parsed.values["keep-caches"],
        "--keep-caches",
        defaultKeepCaches
      ),
