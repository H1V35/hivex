...
use super::{
  APP_CLIENT_VERSION, AtomicOrdering, CANCELLED, Child, Command, CommandExt, Duration,
  ENVIRONMENT_KEYS, ExecutionProfile, HivexError, INITIALIZE_TIMEOUT, Instant, JoinHandle,
  MAX_VERSION_BYTES, NativeError, OsString, Path, PathBuf, Read, Result, Rpc, Stdio, Transcript,
  Value, admit_profile, env, fs, json, launch_arguments, thread,
};
  ...
  env::vars_os()
    .filter(|(key, _)| {
      ...
      ENVIRONMENT_KEYS.iter().any(|allowed| *allowed == key)
        || (key.starts_with("LC_") && key[3..].chars().all(|c| c.is_ascii_uppercase() || c == '_'))
    })
    .collect()
}
...
pub(super) fn launch_server(
  ...
  let native_version =
    run_version(binary, &environment).map_err(|error| NativeError::PreSpawn(Box::new(error)))?;
  let mut command = Command::new(binary);
  command
