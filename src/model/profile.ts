...
const disabledFeatureArguments = function disabledFeatureArguments(
  feature: string
) {
  return ["--disable", feature];
};
const settingArguments = function settingArguments([key, value]: [
  string,
  string,
]) {
  return ["-c", `${key}=${value}`];
...
export const admitProfile = async (options: {
  ...
  const authenticated = account.parse(
    await rpc.request("account/read", {}, { signal })
  );
  const catalog = await readModelCatalog(rpc, signal);
  const advertised = catalog.find(
    (entry) => entry.model === knowledgeModel.name
  );
  ...
  if (
    ...
    if (endpoint.href !== "https://chatgpt.com/") {
      throw new Error(
        "Managed ChatGPT endpoint does not match the admitted provider"
      );
    }
  }
  ...
  if (
    ...
    throw new Error(
      "Configured MCP names cannot be safely overridden by this native profile"
    );
  }
...
export type ProfileEvidence = Awaited<
  ReturnType<typeof admitProfile>
>["evidence"];
...
export const nativeEnvironment = () => {
  ...
  return Object.fromEntries(
    Object.entries(process.env).filter(
      ([name]) => allowed.has(name) || localeKey.test(name)
    )
  );
};
