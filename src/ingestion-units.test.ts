...
const documentOf = function documentOf(text: string, path = 'notes.md'): Document {
  return {
    hash: digest(text),
    historical: false,
    id: path,
...
describe('ingestionUnits', () => {
  ...
  test('retains a large Markdown source in deterministic bounded units', () => {
    const paragraphs = Array.from({ length: 40 }, (...callbackArguments: [undefined, number]) => {
      const [, index] = callbackArguments;
      return [`## Section ${index}`, '', `Paragraph ${index}: ${'word '.repeat(240)}`, ''].join(
        '\n'
      );
    });
    ...
    expect(first.units.every((unit) => Buffer.byteLength(unit.text, 'utf-8') <= 8192)).toBe(true);
  ...
  test('enforces the limit using raw UTF-8 bytes', () => {
    ...
    expect(result.units.every((unit) => Buffer.byteLength(unit.text, 'utf-8') <= 8192)).toBe(true);
  });
  ...
  test.each(['```~', '```ts', '~~~`', '~~~ts'])(
    ...
    (opening) => {
      ...
      expect(result.warnings).toEqual([]);
      expect(result.units.map((unit) => [unit.id, unit.lineStart, unit.lineEnd])).toEqual([
        ['notes.md:1-7', 1, 7],
        ['notes.md:8-8', 8, 8],
      ]);
  ...
  test('keeps paragraphs and fences intact when a boundary can fit', () => {
    const paragraph = function paragraph(letter: string) {
      return Array.from({ length: 18 }, (...callbackArguments: [undefined, number]) => {
        const [, index] = callbackArguments;
    ...
    const fence = [
      '```ts',
      ...Array.from({ length: 120 }, (...callbackArguments: [undefined, number]) => {
        const [, index] = callbackArguments;
        return index === 50
