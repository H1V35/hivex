...
const documentOf = function documentOf(
  text: string,
  path = "notes.md"
): Document {
  return {
...
describe("ingestionUnits", () => {
  ...
  test("retains a large Markdown source in deterministic bounded units", () => {
    const paragraphs = Array.from(
      ...
      (...callbackArguments: [undefined, number]) => {
        ...
        return [
          `## Section ${index}`,
          "",
          `Paragraph ${index}: ${"word ".repeat(240)}`,
          "",
        ].join("\n");
      }
    );
    ...
    expect(
      first.units.every((unit) => Buffer.byteLength(unit.text, "utf-8") <= 8192)
    ).toBe(true);
    expect(first.units.every((unit) => unit.hash === digest(unit.text))).toBe(
      true
    );
    expect(first.units.map(({ id }) => id)).toEqual(
      first.units.map(
        (unit) => `${unit.document}:${unit.lineStart}-${unit.lineEnd}`
      )
    );
  });
  ...
  test("distinguishes duplicate raw content by absolute range", () => {
    ...
    if (!first || !second) {
      throw new Error("Expected two duplicate-content units");
    }
  ...
  test("enforces the limit using raw UTF-8 bytes", () => {
    ...
    expect(
      result.units.every(
        (unit) => Buffer.byteLength(unit.text, "utf-8") <= 8192
      )
    ).toBe(true);
  });
  ...
  test("warns and omits an oversized line while retaining later lines", () => {
    ...
    expect(result.units.map((unit) => unit.text).join("")).toBe(
      "before\nafter\n"
    );
    expect(
      result.units.map(({ lineStart, lineEnd }) => [lineStart, lineEnd])
    ).toEqual([
      [1, 1],
      [3, 3],
    ]);
  ...
  test("keeps paragraphs and fences intact when a boundary can fit", () => {
    const paragraph = function paragraph(letter: string) {
      return Array.from(
        ...
        (...callbackArguments: [undefined, number]) => {
          const [, index] = callbackArguments;
          return `${letter}${index}${letter.repeat(228)}`;
        }
      ).join("\n");
    };
    ...
    expect(result.warnings).toEqual([]);
    expect(result.units.map((unit) => unit.text).join("")).toBe(text);
    expect(result.units.some((unit) => unit.text.includes(paragraphA))).toBe(
      true
    );
    expect(result.units.some((unit) => unit.text.includes(paragraphB))).toBe(
      true
    );
