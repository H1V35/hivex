...
use crate::knowledge::RepairRange;
use crate::knowledge::{self as model};
...
fn bounded(value: Option<&String>, minimum: u64, maximum: u64) -> Result<Option<u64>> {
  ...
  let number = arguments::parse_number(value)
    .filter(|number| {
      number.is_finite()
        && number.fract() == 0.0
        && *number >= minimum.to_f64().expect("bounded integer")
        && *number <= maximum.to_f64().expect("bounded integer")
    })
    .ok_or_else(|| {
      argument(format!(
  ...
  Ok(Some(number.to_u64().expect("validated bound")))
}
...
pub fn operation_for(input: &[String]) -> Result<Operation> {
  ...
  let repair_ranges = repair_ranges(&parsed)?;
  let repair = unique(
    parsed
      .repeated
      .get("repair")
  ...
  let options = Operation {
    ...
    execution: crate::integrations::select(
      values.get("integration").map_or("codex", String::as_str),
      values
        .get("codex")
        .cloned()
...
pub fn command(args: &[String]) -> Result<Value> {
  ...
  if options.command == "update" {
    return Ok(crate::knowledge::update(&mut project, &options, None)?.0);
  }
  if options.command == "review" {
    let implementation = crate::review::capture_implementation(
      &project.root,
...
fn repair_ranges(parsed: &arguments::Parsed) -> Result<Vec<RepairRange>> {
  let mut repair_ranges = Vec::new();
  let expression = regex::Regex::new(r"^(.+):([0-9]+)-([0-9]+)$").expect("valid range expression");
