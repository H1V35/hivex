...
pub fn check_review(arguments: &[String]) -> Result<Value> {
    let parsed = arguments::parse(arguments, &["check", "root"], &[])?;
    ...
    let report_file = report_path(&root, parsed.values.get("check").expect("checked above"));
