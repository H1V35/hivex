...
fn valid_extraction_relationship(entry: &ExtractionRelationship) -> bool {
    !entry.from.is_empty()
        ...
        && !entry.to.is_empty()
        && valid_relationship_type(&entry.relationship_type)
...
fn annotate_graph_serialization(mut graph: Graph, value: &Value) -> Graph {
    ...
    if let Some(Value::Array(records)) = root.get("decisions") {
        for (entry, record) in graph.decisions.iter_mut().zip(records) {
            if record.is_object() {
    ...
    if let Some(Value::Array(records)) = root.get("relationships") {
        for (entry, record) in graph.relationships.iter_mut().zip(records) {
            if record.is_object() {
    ...
    if let Some(Value::Array(records)) = root.get("warnings") {
        for (entry, record) in graph.warnings.iter_mut().zip(records) {
        ...
        .flat_map(|document| {
            document.lines.iter().filter_map(|line| {
                line.first().and_then(|value| match value {
                    Value::Number(number) => number.as_u64().map(|line| line as usize),
...
fn relationship_identity(
    ...
    value.insert(
        "type".to_owned(),
        Value::String(entry.relationship_type.clone()),
    );
    value.insert("reason".to_owned(), Value::String(entry.reason.clone()));
    value.insert(
        "evidence".to_owned(),
...
pub fn apply_extraction(options: ExtractionOptions<'_>) -> Graph {
    let referenced_ids: HashSet<String> = options
        ...
        .collect();
    let mut decisions: Vec<Decision> = options
        .graph
        .decisions
        .iter()
        .filter(|entry| {
    ...
    for entry in &options.extraction.decisions {
        ...
        let located = valid_citation(&citation, options.documents)
    ...
    for entry in &options.extraction.relationships {
        ...
        if missing_endpoint || duplicate || invalid_evidence {
            let message = if missing_endpoint {
                format!("Relationship {} has an unknown endpoint.", entry.id)
            } else if duplicate {
                format!("Relationship {} has a duplicate ID.", entry.id)
            } else {
                format!("Relationship {} has invalid evidence.", entry.id)
            };
            extraction_warnings.push(Warning::Structured(WarningRecord {
                kind: Some("validation".to_owned()),
                message,
    ...
    documents.retain(|id, version| {
        !options.documents.iter().any(|document| {
            document.id == *id && document.hash != version.as_str().unwrap_or_default()
        })
    });
        ...
        .map(|entry| {
            ...
            let uncertain = impact.relationship_ids.contains(&entry.id)
                ...
                || decisions.iter().any(|decision| {
                    (decision.id == entry.from || decision.id == entry.to)
                        && decision.quality == "uncertain"
                });
            Relationship {
                quality: if uncertain { "uncertain" } else { "checked" }.to_owned(),
                ..entry.clone()
