...
fn invalid_evidence_and_local_findings_remain_scoped() {
  ...
  r["check"]["findings"] = json!([{"target":"c3","reason":"The decorative exception is unclear."}]);
  p.json("responses.json", &r);
...
fn legacy_pre_spawn_failure_preserves_pending_check_and_attempt_history() {
  ...
  w["attempts"].as_array_mut().unwrap().push(
    json!({"inputBytes":1,"inputHash":"legacy-admission-failure","report":report,"stage":"check"}),
  );
